# -*- coding: utf-8 -*-
"""
Created on Sat Jan 16 21:32:58 2021
...
command.execute('Create table if not exists Billing(BID int primary key,\
               ID int, R_no int, Enroll_Date date, DisEnroll_Date date,Amount int, GST char(10), Net_Amount int)')
...
def RemoveVisitor():
    command.execute('Select * from Hotel_Details')
    command.fetchall()
    cnt=command.rowcount
    if cnt==0:
        print()
...
def ShowBilling_Details():
    ...
    if cnt==0:
        ...
        df=pd.DataFrame(billing_details, columns=['Booking ID', 'Guest ID', 'Room No', 'Enroll_Date', 'DisEnroll_Date',  'Amount', 'GST',  'NetAmount'])
        print(df.to_string(index=False))
    print('**********************************************************')
...
def BookingRoom():
    ...
    while True:
        command.execute('Select * from Guest_details where ID={}'.format(ID))
        command.fetchall()
        cnt=command.rowcount
        if cnt==0:
    ...
    command.execute('Select Name from Guest_Details where ID={}'.format(ID))
...
def Report_IncomeFromBooking():
    ...
    while True:
        ...
        if cnt<=0:
            ...
            command.execute('Select * from Previous_Booking where Year(Check_out_Date)={}'.format(Year,))
            command.fetchall()
            cnt=command.rowcount
            if cnt==0:
                print('__________________________________________________________')
